...
function renderTable() {
        ...
        tbody.innerHTML = `
            <tr><td colspan="8">
                <div class="empty-state">
                    ...
                    <p>${templates.length === 0 ? 'No templates yet. Click <strong>+ New Template</strong> to create one.' : 'No templates match your filter.'}</p>
                </div>
            </td></tr>`;
        return;
    }
    ...
    tbody.innerHTML = filtered.map((t, i) => {
        ...
        <tr>
            ...
            <td>${typeBadge}</td>
            <td>
                <span style="font-weight:600;color:black;">${t.rules?.length || 0}</span>
                <span style="font-size:11px;color:black;"> rules</span>
            </td>
            <td>${fileChip}</td>
            <td>${statusBadge}</td>
            ...
            <td class="action-cell">
                ...
                <button class="btn-action" onclick="openEditModal(${t.id})">✏️ Edit</button>
                <button class="btn-action danger" onclick="openDeleteModal(${t.id})">🗑</button>
            </td>
        </tr>`;
    }).join('');
}
