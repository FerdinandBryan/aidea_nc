// Librarian_thesis_submissions.js
// Librarian version of the admin Thesis Submissions page: same API data, same
// review + repository features, same theme / drawer / profile menu / sign-out.
(function () {
    'use strict';
    ...
    // Roles allowed on this page. If the stored user has a `role` field that is
    // not in this list, the visitor is sent back to the login page.
    const ALLOWED_ROLES = ['librarian'];
    ...
    // ── Session helpers (same keys as the admin panel) ─────────────────────
    function getToken() {
    ...
    function isAllowed(user) {
        ...
        if (!user || !user.role) return true; // no role info stored: let the API decide
        return ALLOWED_ROLES.indexOf(String(user.role).toLowerCase()) !== -1;
    }
    function renderLibrarianIdentity(user) {
        const fullName = [user.fname, user.lname].filter(Boolean).join(' ') || 'Librarian';
    ...
    function initProfileMenu() {
        ...
        if (signOutBtn) signOutBtn.addEventListener('click', function () {
            ...
            openConfirm({
                title: 'Sign out?',
                message: 'You will need to log in again to use the library panel.',
    ...
    async function loadTheses() {
        try {
            ...
            if (res.status === 403) {
                showToast('Your account does not have access to thesis submissions.', 'error');
                return;
            }
    ...
    function viewThesis(id) {
        ...
        if (t.file) {
            fileSection.innerHTML =
                ...
                '<button class="btn-file" onclick="LibrarianThesis.checkFile(' + t.id + ')">View</button>' +
                '<button class="btn-file" onclick="LibrarianThesis.downloadFile(' + t.id + ')">Download</button>' +
                '</div>' +
                '</div>';
        } else {
    ...
    function renderRepoModal() {
        ...
        listEl.innerHTML = '<div class="repo-list">' + filteredRepo.map(function (p, i) {
            return '<div class="repo-item">' +
                ...
                ((p.file && p.file.url) ? '<a class="btn-repo-link" href="' + escHtml(p.file.url) + '" target="_blank" rel="noopener">View</a>' : '') +
                '<button class="btn-repo-remove-sm" onclick="LibrarianThesis.removeFromRepository(' + p.id + ')">Remove</button>' +
                '</div>' +
                '</div>';
        }).join('') + '</div>';
    }
    ...
    function repoBtnHtml(thesis) {
        ...
        if (isInRepository(thesis.id))
            return '<button class="btn-repo btn-repo-remove" onclick="LibrarianThesis.removeFromRepository(' + thesis.id + ')">Remove</button>';
        return '<button class="btn-repo btn-repo-add" onclick="LibrarianThesis.addToRepository(' + thesis.id + ')">+ Repo</button>';
    }
    ...
    function render() {
        ...
        tbody.innerHTML = filtered.map(function (t) {
            return '<tr>' +
                ...
                '<button class="btn-action" onclick="LibrarianThesis.viewThesis(' + t.id + ')">View</button>' +
                '<button class="btn-action" onclick="LibrarianThesis.reviewThesis(' + t.id + ')">Review</button>' +
                repoBtnHtml(t) +
                '</td>' +
                '</tr>';
        }).join('');
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (user) renderLibrarianIdentity(user);
